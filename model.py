...
class MobileNet:
    ...
    def __init_output(self):
        with tf.variable_scope('output'):
            ...
            #tf.cast(self.y_out_argmax, tf.int32)
            #tf.cast(self.y, tf.int32)
            #print('type:', type(self.y_out_argmax))
            #print('type:', type(self.y))
            k=tf.equal(self.y, self.y_out_argmax)
            self.accuracy = tf.reduce_mean(tf.cast(k, tf.float32))
