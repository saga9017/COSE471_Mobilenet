...
def __conv2d_p(name, x, w=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
    ...
    #print('feature의 shape', x.shape)
    return out

def hidden(name, x, w=None, w_new=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
               initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0, bias=0.0):

    with tf.variable_scope(name):
        kernel_shape_new=[4,4,(x.shape[3])//8,1 ]
        y=tf.reshape( x, [-1, (x.shape[1])*2, (x.shape[2])*4, (x.shape[3])//8])
        with tf.name_scope('layer_weights'):
            if w_new==None:
                w_new = __variable_with_weight_decay(kernel_shape_new, initializer, l2_strength)
            __variable_summaries(w_new)
        with tf.name_scope('layer_biases'):
            if isinstance(bias, float):
                bias = tf.get_variable('biases', [num_filters], initializer=tf.constant_initializer(bias))
            __variable_summaries(bias)
        with tf.name_scope('layer_conv2d'):
            conv_new=tf.nn.depthwise_conv2d(y, w_new, stride, 'VALID')
    return conv_new

def hidden(name, x, w=None, w_new=None, stride=(1, 1),
               initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0, bias=0.0):

    with tf.variable_scope(name):
        stride = [4, 4, 4, 4]
        kernel_shape_new=[4,4,(x.shape[3])//16,1 ]
        print('x의 shape', x.shape)
        y=tf.reshape( x, [-1, (x.shape[1])*4, (x.shape[2])*4, (x.shape[3])//16])
        ...
        with tf.name_scope('layer_biases'):
            if isinstance(bias, float):
                bias = tf.get_variable('biases', [y.shape[-1]], initializer=tf.constant_initializer(bias))
            __variable_summaries(bias)
        with tf.name_scope('layer_conv2d'):
            conv_new=tf.nn.depthwise_conv2d(y, w_new, stride, 'VALID')
            out = tf.nn.bias_add(conv_new, bias)
    print('out의 shape :', out.shape)
    return out

def __conv2d_p_new(name, x, w=None, w_new=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
               initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0, bias=0.0):
    """
    Convolution 2D Wrapper
    :param name: (string) The name scope provided by the upper tf.name_scope('name') as scope.
    ...
    with tf.variable_scope(name):
        ...
        with tf.name_scope('layer_weights'):
            if w == None:
                ...
                #print('new kernel_shape :',kernel_shape_new )
            __variable_summaries(w)
        with tf.name_scope('layer_biases'):
            if isinstance(bias, float):
                bias = tf.get_variable('biases', [num_filters], initializer=tf.constant_initializer(bias))
...
def __depthwise_conv2d_p(name, x, w=None, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
    ...
    with tf.variable_scope(name):
        ...
        with tf.name_scope('layer_conv2d'):
            ...
            out = tf.nn.bias_add(conv, bias)
    #print('w의 shape :', w.shape)
    #print('input :', x.shape, 'depthwise_conv의 결과 :', conv.shape)
    return out
...
def depthwise_separable_conv2d(name, x, w_depthwise=None, w_pointwise=None, width_multiplier=1.0, num_filters=16,
    ...
    with tf.variable_scope(name) as scope:
        ...
        conv_h=hidden('hidden', x=conv_a, w=None, w_new=None, stride=(1, 1),
               initializer=initializer, l2_strength=l2_strength, bias=0.0)
        conv_o = conv2d('pointwise', x=conv_h, w=w_pointwise, num_filters=total_num_filters, kernel_size=(1, 1),
                        initializer=initializer, l2_strength=l2_strength, bias=biases[1], activation=activation,
                        batchnorm_enabled=batchnorm_enabled, is_training=is_training)
    with tf.Session() as sess:
        a, o=sess.run([tf.shape(conv_a), tf.shape(conv_o)])
        print(name, "의 conv_a의 shape :", a)
        print(name, "의 conv_o의 shape :", o)
